  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 shadow-sm p-6 transition-colors">
      <h2 className="text-xl font-bold mb-4 flex items-center gap-2 dark:text-white">
        <CalendarDays className="w-5 h-5 text-gray-700 dark:text-gray-300" /> 今週のスケジュール
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-7 gap-2">
        {weekDays.map((date, i) => {
          ...
          return (
            <div key={i} className={`flex flex-col md:h-full md:min-h-[150px] min-h-[80px] border dark:border-gray-700 rounded-lg p-2 transition-colors ${isToday ? 'bg-gray-50 dark:bg-gray-700/50 ring-1 ring-gray-300 dark:ring-gray-500 border-transparent' : 'bg-gray-50 dark:bg-transparent'}`}>
              <div className={`text-left md:text-center mb-3 text-sm font-bold flex justify-between md:flex-col md:items-center items-center gap-1 ${i === 0 ? 'text-red-500 dark:text-red-400' : i === 6 ? 'text-blue-500 dark:text-blue-400' : 'text-gray-700 dark:text-gray-300'}`}>
                ...
                <span className={`w-8 h-8 flex items-center justify-center rounded-full ${isToday ? 'bg-black dark:bg-white text-white dark:text-black' : ''} text-lg`}>{date.getDate()}</span>
              </div>
              <div className="flex flex-col gap-2 flex-1">
                {dayEvents.map(event => (
                  <Link href={`/companies/${(event as any).company_id || '#'}`} key={event.id} className="block group">
                     <div className="bg-white dark:bg-gray-800 p-2 rounded border-l-4 border-l-gray-800 dark:border-l-gray-200 border-y border-r border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md hover:scale-[1.02] transition-all">
